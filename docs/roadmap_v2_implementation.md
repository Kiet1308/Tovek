...
## R2: source binding preservation and statement/value ancestry

R2 now constrains presentation before SSA destruction: a private, uncaptured conditional result that is both observed and returned keeps its own binding when selecting between two distinct incoming parameters. Recorded source names and stronger existing names take precedence over the inferred `selected` role. Optional argument normalization keeps its parameter role; existing liveness, effect, capture and local-pressure gates remain authoritative. [Contract and limits](source_binding_preservation.md).

The optional trace records immutable nested input occurrences, explicit phi transport and committed inline events. Statement and compound-value tags survive moves and actual AST copies; reductions merge origins. The formatter emits exact output spans, including named closures, with node ancestry separate from storage dependency ancestry. Twenty-three pass families declare preservation/invalidation policies. Compiler loop temporaries require loop-protocol evidence; missing names alone never establish synthesis or temporary status. Scalar leaves and unattributed replacements can remain unknown.

All 1,018 primary Rust tests plus one child repeat, 122 Python tests, 198 runtime profiles, 513 public profiles, 42 compiler-witness profiles, 45 legacy semantic profiles and 52 size gates pass. The new binding fixture detects all 24 compiled mutants across six profiles, including false/nil selection, optional argument normalization and captured-cell observations. Runtime dataflow retains 119 proved, 27 different and 52 unknown results despite VM agreement; the six added profiles are unknown. The 84-profile reconstruction study passes in both modes and detects all 84 mutants in each mode; its unblinded status remains unchanged.

Private output changes in 28 of 3,978 files, involving 31 binding renames. Every change preserves the parser's lexical binding/type shape and recompiles at O0/O2; no specific existing name becomes generic `selected`. All 3,936 nonempty scripts pass independent parser/binding checks. Default/detailed source and complete sidecars at 1/16 threads agree. Runtime/public replay also passes compact-annotation, capture, metadata and cold/warm cache checks. The persistent-node metadata itself changes none of the source outputs from the preceding R2 consumer build.

Private coverage includes 2,318,516 initial nested values and 1,358,916 output regions: 645,092 retain node ancestry, 680 record explicit synthesis, and 713,144 remain unknown, with no omitted output regions. Storage ancestry and inline/copy flags are separate, overlapping counts; none claims an exact final producer instruction or recovery of the author's original variables. These complete the bounded R2 mechanisms and acceptance rows. R7 remains deferred, R9 paused and AI off by default. Only source, owned examples, compact reports and hashes are archived. [Acceptance inventory](roadmap_v2_acceptance/source_binding_validation.json).

## R4/R5: ordered expression motion and bounded reconstruction

R4's remaining rows now use explicit evaluation events for late AST/UI motion, safe single-use field/scalar-call aliases, existing curried/private-constructor ownership rules and bounded exact-value facts from runtime guards. R5 retains prospective helper binders through SSA, matches scalar let/phi/return regions earlier, prioritizes search with input line/PC hints, independently protects eager arguments and captures, and records equivalent-call inference separately from synthesis. Candidate definitions remain intact under ambiguity; unused-parameter patterns cannot veto optional loop synthesis. [Contract and limits](ordered_reconstruction.md).

All 1,007 primary Rust tests plus one child repeat, 117 Python tests, 192 runtime profiles, 513 public profiles, 42 original compiler-witness profiles, 45 legacy semantic profiles and 52 size gates pass. The new 192-vector fixture detects all 42 compiled mutants. The preceding release fails its captured-cell observation at O0/O1/O2 g1; the new release passes all six profiles. Four prior conditional dataflow results change from proved to different after guard refinement, despite VM agreement; those classifications remain visible.

Private source changes in 493 of 3,978 files, with 347 changes beyond reconstruction annotation text. All 15 remaining SETLIST packing fallbacks disappear in this corpus as eligible trees are rebuilt. Some former helper calls expand to statements because reference-captured arguments lack the required snapshot proof. Every changed source and seven residual fixtures parse/recompile at O0/O2; all 3,936 nonempty scripts pass independent parser/binding checks. Final source and complete detailed sidecars agree with the audited artifact and at 1/16 threads; default/detailed sources agree. Runtime/public default, compact, capture/emission and cold/warm cache checks pass.

Sixteen public development outputs change, with all dataflow statuses and all 45 holdout outputs unchanged. Fusion sRGB O2 restores `transform`/`inverse`, raising its raw structural ratio from 0.294 to 0.652; this is one profile, not a general fidelity gain. The fourteen-family study passes all 84 profiles in both modes and detects 84 compiled mutants. Source-call precision/recall on its first holdout is 0%/0%; after unblinding it is 60%/37.5% on the specific reviewed sites. Only the unit-stride loop family is synthesized with opt-in. [Site alignment, denominators and research refusals](reconstruction_study.md).

These complete the bounded R4/R5 implementation and research rows, without claiming recovery of arbitrary original calls or loops. R7 remains deferred and R9 paused; AI stays off by default. A full disk interrupted preliminary checks; reruns passed after Cargo-cache cleanup and NTFS compression of generated output. No models, binaries, compiler profiles or private source/bytecode are published. [Acceptance inventory and reproducible galleries](roadmap_v2_acceptance/ordered_reconstruction_validation.json).
