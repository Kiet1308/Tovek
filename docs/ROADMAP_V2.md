...
- [x] Nền tảng effect của expression có budget: phân biệt total-pure, may-throw, allocation, call/yield và read/write của table/global/capture; unknown giữ rào cản, không dùng type hint làm purity proof. Chưa bao phủ toàn bộ statement/store/alias graph. [Phạm vi và nghiệm thu](captured_evaluation_order.md).
