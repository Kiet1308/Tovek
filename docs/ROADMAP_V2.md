...
- [x] Gắn origin ở độ hạt statement/value: prototype, PC range hoặc PC set, vai trò instruction và line range khi có. Origin sinh từ nhiều nơi phải được biểu diễn là nhiều nơi. — Graph đầu vào ghi cả nested value và địa chỉ LHS; statement/compound node giữ tag qua transform, reduction hợp nhất nhiều origin có budget. PC/line thuộc instruction cluster; scalar lá ở output có thể vẫn unknown, không công bố exact producer PC.
- [x] Giữ quan hệ definition SSA ↔ register interval ↔ source/debug binding qua inline, clone, phi elimination và local map. — Ghi substitution đã commit, tag cho AST copy thực tế và map phi parameter/edge transport; debug interval và source identity tách khỏi ancestry của storage.
- [x] Tạo phân loại local có bằng chứng: parameter, source local đã biết, result của select, iteration binding, capture cell, compiler temporary và synthesized local. — Vai trò lexical và ancestry được báo riêng; compiler temporary chỉ nhận khi có marker chuẩn bị numeric/generic-for. Thiếu debug name không đủ bằng chứng; producer chưa ghi nhận giữ unclassified.
- [x] Ghi local tổng hợp do hai pass cuối tạo ra, gồm selected result, short-circuit result và snapshot; chỉ công bố rewrite đã commit, có budget và nối tới identifier/khai báo lexical bằng ID. Không suy synthesis từ thiếu debug/SSA, không sao chép nguồn hay chứng nhận lifetime. Các producer khác vẫn chưa xác định. [Hợp đồng và nghiệm thu](emitter_local_origins.md).
- [x] Xuất graph khai báo lexical riêng với storage ID: vai trò cú pháp, capture trong output, recorded origins và ancestry được tách rõ; shared storage giữ ambiguous, không suy ra temporary/synthesis từ việc thiếu debug name. Kiểm tra 168 runtime, 513 public và 3.936 private script. Đây là lớp diagnostic, chưa đổi quyết định coalescing. [Hợp đồng và nghiệm thu](lexical_binding_graph.md).
- [x] Tách quyết định tái sử dụng storage khỏi quyết định dùng cùng một tên/khai báo nguồn. Áp lực local của Luau vẫn là ràng buộc, nhưng không phải tiêu chí duy nhất. — `BindingRoles` độc lập với `SourceBinding` và trace; compatibility xét toàn bộ lớp coalescing, giữ ràng buộc result qua phi transport. Consumer giới hạn 512 CFG node/160 local để chừa headroom.
- [x] Bảo toàn source binding mạnh trước các phép inline/coalescing; với input strip, dùng role/liveness/effect facts để đề xuất split và kiểm chứng từng ca. Không tự tách một parameter đang được gán rõ ràng chỉ để tăng số local hoặc ép một initializer. — Select private, uncaptured, được đọc nhiều lần và return mới nhận role; chỉ buộc tách khỏi parameter khi chọn giữa hai parameter đầu vào khác nhau. Normalization đối số tùy chọn giữ vai trò cũ; liveness/effect/capture gate hiện có vẫn quyết định rewrite.
...
- [x] Mở rộng source map hiện có từ closure occurrence tới vùng output/PC khi mapping có nghĩa; ghi rõ vùng inlined, cloned hoặc synthesized. — Formatter ghi span UTF-8 thực tế cho statement/value và cả named closure; node ancestry tách khỏi storage dependency, ghi inline/copy/synthesis đã xảy ra và unknown rõ ràng. Copy AST không đồng nghĩa đánh giá expression nhiều lần ở runtime.
- [x] Quy định mỗi pass phải giữ, hợp nhất hay vô hiệu hóa loại provenance/proof nào. Không chuyển một close/ownership certificate sang local mới chỉ vì tên giống nhau. — Inventory 23 nhóm pass xuất cùng sidecar; move giữ tag, copy đánh dấu, reduction merge, node mới chưa gán nguồn giữ unknown. Effect/lifetime proof có hợp đồng độc lập.
...
Đã nghiệm thu toàn bộ cơ chế R2 với giới hạn nêu trên: 1.018 Rust test chính, 122 Python test, 198 runtime profile, 513 public profile, 3.936 private script và 52 size gate đều qua. Output private đổi tên ở 28/3.978 file, không có tên cụ thể bị thay bằng `selected`; source và sidecar giống nhau ở 1/16 luồng. Coverage công bố cả 713.144/1.358.916 vùng private có node ancestry unknown; không coi ancestry là khôi phục chính xác biến hay producer gốc. [Hợp đồng, coverage và cách chạy lại](source_binding_preservation.md), [bằng chứng nghiệm thu](roadmap_v2_acceptance/source_binding_validation.json).
...
- [x] Hạ result dùng nhiều lần thành phép gán trong nhánh, hoặc local riêng được gán bằng `if/else` khi cần. Giữ `return` theo nhánh khi rõ ràng; không tạo initializer `if` expression hay nhân đôi expression có effect để loại local. — Phi/branch hiện có giữ statement; conditional IR scalar được hạ một lần, giữ binding/điểm khai báo cũ và kiểm tra nhiều lần đọc, nil/false, capture, arity và budget. [Phạm vi và nghiệm thu](conditional_ir_lowering.md).
- [x] Phân tích effect tinh hơn: pure và total, may-throw, table read/write, allocation, call, yield và access tới captured cell. Unknown vẫn là rào cản. — Summary expression có budget, không dùng type hint làm purity proof; statement/store/alias dependency graph là mục riêng bên dưới. [Phạm vi và nghiệm thu](captured_evaluation_order.md).
...
- [x] Giữ thứ tự đối số `__eq` khi SSA inline so sánh; chỉ đảo equality nếu một vế là primitive literal đủ loại trừ metamethod. Fixture khóa kết quả/lỗi/call order và đối chiếu Roact `createSpy`. [Bằng chứng và phạm vi](equality_order.md).
- [x] Giữ captured read trước vị trí thay thế ở callee/argument/return tuple/LHS base-key khi candidate có thể ghi cell; cho phép read/read và incoming VAL/UPVAL đã có chứng nhận immutable từ mọi constructor. Verifier riêng đọc bytecode gốc; REF và thiếu proof vẫn chặn. [Bằng chứng, chi phí và fallback](captured_evaluation_order.md).
- [x] Giữ initializer trước nested index ở LHS base/key trong late AST/UI inline; phân biệt các phép đọc địa chỉ trước RHS với store cuối cùng sau RHS. Kiểm tra lỗi, mutation/capture, nil/NaN key và scalar/multret; index lá vẫn được inline khi đủ proof. [Hợp đồng và nghiệm thu](assignment_evaluation_order.md).
- [x] Gắn effect dependency theo vị trí đánh giá callee, argument, LHS base/key, RHS và store. Quyết định inline phải dựa trên quan hệ này. — Chuỗi event có budget dùng trực tiếp trong late AST/UI inline; SSA giữ các guard thứ tự đã nghiệm thu. [Phạm vi và fallback](ordered_reconstruction.md).
- [x] Dựng constructor cho diamond liền kề, mỗi nhánh ghi cùng một property literal của bảng private; snapshot initializer theo thứ tự, giữ `if/else`, recorded binding và layout callback. `branch_ui` g1 có nhóm props/children rõ hơn; g2 giữ nguyên. Có 1.470 vector trên sáu profile và 36 mutant controls. [Phạm vi và nghiệm thu](branch_constructors.md).
- [x] Mở rộng vùng constructor qua khai báo selected local hoặc statement xen kẽ khi có dependency/alias proof. — Bảng private chưa bị đọc/capture, initializer literal/local ổn định, kiểm tra cả hai nhánh và LHS base/key; tối đa 64 statement/4.096 node/độ sâu 32. Tái dựng constructor ở 50 file private, giảm SETLIST packing từ 23 xuống 15 site ở 14 file; các ca chưa đủ proof vẫn giữ fallback. [Hợp đồng, ví dụ và nghiệm thu](constructor_regions.md).
- [x] Khôi phục cây curried UI/callback theo các rule cấu trúc và ownership đã có; thư viện như Fusion/Roact cung cấp ngữ cảnh, không tự tạo ngoại lệ về semantics. — Kết hợp rule curried hiện có, vùng constructor private và event-order gate; không thêm ngoại lệ API. [Hợp đồng](ordered_reconstruction.md).
- [x] Loại alias dùng một lần trong những ca field/call đã chứng minh giữ thứ tự, như một số method của `BufferWriter`; giữ alias khi nó chụp một giá trị trước mutation. — Giữ recorded binding, require/GetService header, captured snapshot và scalar tail; lookup global trước đối số vẫn có thể buộc giữ alias của BufferWriter.
- [x] Dùng facts từ runtime guard khi cần chứng minh thuộc tính giá trị. Annotation/hint kiểu không tự chứng minh tính total hay không có NaN. — Facts equality cho primitive có giới hạn; không suy từ truthiness, ±0, captured cell hay type annotation. [Test và giới hạn](ordered_reconstruction.md).
...
- [x] Tạo bộ ví dụ compiler cho các họ: statement/expression inlining, constant argument specialization, result alias, early return, fixed-count loop unroll và table lowering. Pin compiler/flags của từng họ. — Bảy họ với 42 image bytecode khóa trước khi đánh giá decompiler, 162 vector runtime/profile, 42 mutant compile được và bị phát hiện; giữ riêng 28 proved, 13 different và 1 unknown. [Witness và giới hạn](compiler_transformation_witnesses.md).
- [x] Giữ candidate helper gắn với prototype/capture identity; dùng line/PC provenance để tìm vùng nghi ngờ trước khi mở search trên mọi subtree. — Giữ binder scalar qua SSA; line/PC chỉ ưu tiên candidate, không loại rival hay chứng nhận call-site gốc. [Hợp đồng](ordered_reconstruction.md).
- [x] Chuẩn hóa candidate ở dạng dataflow/region phù hợp, với phi/select và return arity còn rõ. Không yêu cầu helper và call-site đã đi qua cùng một chuỗi AST cleanup mới nhận ra được nhau. — Scalar let/return/phi region có budget, giữ binding kết quả dùng nhiều lần; không nhận mọi specialization.
...
- [x] Giữ argument evaluation count/order và capture binding khi phục hồi call. Một argument được dùng nhiều lần hoặc chỉ dùng trong một nhánh không được tự động chuyển thành eager evaluation. — Census capture độc lập cho các matcher; prefix copy giữ thứ tự, dependency và single-result adjustment. Một số call cũ bị từ chối lại vì thiếu proof.
- [x] Nghiên cứu re-roll loop có số lượt cố định, index thay đổi đều và body tương ứng. Giữ thứ tự operator/effect, giá trị induction, result/capture và ngữ nghĩa zero/last iteration. — Nghiên cứu thêm stride 2, reverse, zero, last-result, capture và effect bằng compiler/VM witness; giữ refusal ngoài family tổng 4–8 lượt đã có proof. [Kết quả và giới hạn](reconstruction_study.md).
...
- [x] Với ca không còn helper prototype hoặc có nhiều source candidate cùng hợp lệ, phân loại là suy luận/synthesis; không cộng vào số helper gốc đã khôi phục chắc chắn. — Schema 2 phân biệt equivalent-call inference/synthesis; thiếu prototype hoặc nhiều helper hợp lệ thì không đoán helper gốc. Handwritten shape được tính false positive khi đo dự đoán source-call.
- [x] Thử recompile candidate trong đúng context/compiler profile như một kiểm chứng bổ sung. Không dùng oracle bỏ register hiện tại làm chứng nhận cho candidate. — Recompile toàn module của 42 witness và 84 profile nghiên cứu ở đúng O/g/flags; giữ riêng runtime, dataflow và source-call alignment.
- [x] Đặt budget cho số candidate, node, thời gian và bộ nhớ; có đường quay về output đã kiểm chứng khi hết budget hoặc proof không đủ. — Giới hạn cây/payload/candidate và work fuel xác định, không commit khi hết budget giữa ambiguity scan; harness có timeout. Không cam kết hard wall-clock/RSS cho toàn decompiler. [Các mức giới hạn](ordered_reconstruction.md).
...
Các hàng R4/R5 trên hoàn tất trong phạm vi có giới hạn đã công bố, không có nghĩa mọi call/loop gốc đều khôi phục được. [Nghiên cứu source-call](reconstruction_study.md) giữ kết quả holdout đầu tiên và tách riêng số đo sau khi dùng tập đó để sửa thuật toán. AI/R9 vẫn tạm dừng; tối ưu performance R7 để sau theo yêu cầu người dùng.
...
- [x] Tách việc tắt một rewrite tạo `if` expression khỏi việc hạ conditional expression đã tồn tại trong IR. Với trường hợp nằm trong call argument, return tuple hoặc loop condition, phải giữ vị trí/thứ tự đánh giá, skipped branch và multret; chưa chứng minh được thì giữ cấu trúc statement ở bước trước. — Pass scalar cuối pipeline, 240 cấu hình IR và 18 mutant control; v9, có budget và refusal cho constructor/store, repeat/continue và captured register có thể đổi cách đọc sau O2 inline. Pipeline hiện tại giữ các vùng đó ở statement từ bước trước; IR ngoài phạm vi không bị rewrite ép. [Hợp đồng và coverage](conditional_ir_lowering.md).
...
- [x] Đưa annotation của de-inline/proof/synthesis ra metadata có vị trí; hỗ trợ cách hiển thị gọn trong source. Giữ khả năng truy vết provenance của từng call-site. — Ledger ghi lần tạo call và vị trí output thực, phân biệt prototype helper với PC caller chưa biết; clone giữ event, vùng opaque/đổi node/hết budget không được suy nguồn. `--compact-annotations` cần provenance để giữ text gốc; mặc định không đổi source. Proof ngữ nghĩa vẫn là kênh riêng. [Hợp đồng và nghiệm thu](call_reconstruction_annotations.md).
...
- [ ] Thêm benchmark in-memory API, CLI I/O, cold/warm cache và nhóm file nhỏ/lớn; ghi build hash, options, CPU, thread count, phân bố và số mẫu. — API/CLI warm-cache và artifact cache đã đo; OS cold-cache còn mở.
- [x] Benchmark API trong bộ nhớ và CLI warm-cache trên toàn corpus, nhóm nhỏ/lớn và bốn file đại diện; khóa input/output/build/options, chạy 1/16 thread, giữ từng mẫu và ownership/RSS contract. [Số đo và cách tái lập](api_benchmark.md).
- [x] Đo allocation count/bytes và peak RSS trên ít nhất `Write`, `LightningCore`, `Promise`, một UI lớn và toàn corpus. — Bản đếm mimalloc riêng, 1 thread, first/repeated call, mọi output khớp hash. Số đếm chưa chứng minh allocation-bound; requested bytes không phải RAM hay bytes copied. [Phạm vi và kết quả](api_benchmark.md).
...
- [x] Thử PGO bằng tập train đại diện, đánh giá trên tập giữ riêng, build sang target riêng và giữ unwind recovery. Đo Windows native và deployment target khác riêng biệt. — Đã đo Windows native và Linux/WSL2 riêng trên cùng máy; Windows chưa đạt ngưỡng median 10%, Linux đạt các gate trong lượt đo này. Source/sidecar và panic recovery giữ nguyên; PGO vẫn là thí nghiệm, không bật mặc định. [Phạm vi và toàn bộ số đo](pgo_experiment.md).
